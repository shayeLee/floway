	...
	*/
	...
	var distributor$ = {
	  ...
	  next: function next(events) {
	    ...
	    var obs$ = _config.preObservable.pipe(last());
	    this.subscription = obs$.subscribe(function () {
	      _distributor$.next(events);
	    });
	  }
	};

	var promiseOf = function promiseOf(p) {
	  return _config.preObservable.pipe(last(), switchMap(function () {
	    return fromPromise_2(p);
	  }));
	};
	...
	var _permeate = function _permeate(observablesMap, param1, param2) {
	  ...
	  var handler = function handler(Comp) {
	    ...
	    if (!_isObject$1(observablesMap)) throw new TypeError("\u65B9\u6CD5permeate()\u7684\u53C2\u6570observablesMap\u5FC5\u987B\u662Fobject\u7C7B\u578B");
	    ...
	    var Permeate = function (_React$PureComponent) {
	      ...
	      _createClass(Permeate, [{
	        ...
	        value: function componentDidMount() {
	          ...
	          var init$ = from(_initObservers).pipe(combineAll_2());
	          var initSubscription = init$.subscribe(function (dataArr) {
	            var stateObj = {};
	            dataArr.forEach(function (data) {
	              if (!_isObject$1(data)) throw new TypeError("init observable 推送的数据必须是object类型");
	...
	function getPromise1() {
	    console.log(js_cookie.get("test"));
	    return new Promise(function (resolve) {
	        setTimeout(function () {
	            resolve("test");
	        }, 1000);
	...
	var source$ = promiseOf(getPromise1()); /* attract("test").pipe(
	                                        switchMap(() => {
	...
	var App = permeate({ source: source$ })(_app);
